...
// for guest enquiry form submission
function putOrganizationCampusToSession() {
    let _id = document.getElementById('organization_campus_id').value;
    $.ajax({
        url: $('.appUrl').val()+'/openEnquiries/storeOrganizationCampusSession/' + _id,
        type: 'get',
...
function appendSDashForR(event, prefix) {
    let value = $(event.target).val().replace(prefix,'');
    $(event.target).val('');
    $(event.target).val(prefix+value);
}
...
function delete_enquiry(id) {
    swal({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: $('.appUrl').val()+"/enquiries/" + id + '/delete',
                // dataType: "json",
                method: "GET",
                headers: {
...
function getOrganizationCampusesForProspect(count) {
    $.ajax({
        url: $('.appUrl').val()+'/enquiries/getOrganizationCampuses/',
        type: 'get',
        success: function(data) {
            var campuses = "<div class='form-group'><strong>Organization Campus:<span class='required-span'>*</span></strong>";
            campuses += "<select id='prospect_campus_" + count + "' class='form-control select2 item-required' errorLabel='Prospect Organization Campus " + (count + 1) + "' onchange='getCoursesBySessionForProspect(" + count + ")'>";
            campuses += "<option value=''>--- Select Organization Campus ---</option>";
            $.each(data.campuses, function(index, value) {
                campuses += "<option value='" + index + "'>" + value + "</option>";
            });
            campuses += "</select></div>";
            $("#prospect_campus_div_" + count).html(campuses);
            $('.select2').select2();
        },
        error: function(data) {
            document.getElementById('report_loading').hidden = true;
...
function getCoursesBySessionForProspect(count) {
    selected_session_id = document.getElementById('session_id').value;
    selected_organization_campus = document.getElementById('prospect_campus_' + count).value;
    if (selected_session_id) {
        $.ajax({
            url: $('.appUrl').val()+"/courses/getCoursesBySession/" + selected_session_id + "?organization_campus=" + selected_organization_campus,
            // dataType: "json",
            type: "GET",
            success: function(data) {
                var prospect_course_select = "<div class='form-group'><strong>Courses:<span class='required-span'>*</span></strong>";
...
function getAffiliatedBodiesByCourseForProspect(count) {
    ...
    selected_organization_campus = document.getElementById('prospect_campus_' + count).value;
    if (selected_course_id && selected_session_id && selected_organization_campus) {
        document.getElementById('report_loading').hidden = false;
        $.ajax({
            url: $('.appUrl').val()+"/courses/getAffiliatedBodiesByCourse?session_id=" + selected_session_id + '&course_id=' + selected_course_id + '&organization_campus=' + selected_organization_campus,
            // dataType: "json",
            type: "GET",
            success: function(data) {
                document.getElementById('report_loading').hidden = true;
                var affiliated_body_select = "<div class='form-group'><strong>Affiliated Body:<span class='required-span'>*</span></strong>";
...
function getDegreeTypesByBodyForProspect(count) {
    ...
    if (selected_course_id && selected_session_id && selected_affiliated_body_id) {
        document.getElementById('report_loading').hidden = false;
        $.ajax({
            url: $('.appUrl').val()+"/courses/getDegreeTypesByBody?session_id=" + selected_session_id + '&course_id=' + selected_course_id + '&affiliated_body_id=' + selected_affiliated_body_id + '&organization_campus=' + selected_organization_campus,
            // dataType: "json",
            type: "GET",
            success: function(data) {
                document.getElementById('report_loading').hidden = true;
...
//         document.getElementById('pwwbFileInformationDiv').hidden = true;
//         document.getElementById('prospects_id').classList.remove('item-required');
//         $("#worker_details").html("");
//         document.getElementById('pwwbFileInformationDiv').hidden = true;
//     }

//     if (selected_worker_id == 3) {
//         document.getElementById('pwwbFileInformationDiv').hidden = true;
//     }
// }
...
function workerSectionDelete(index) {
    if ((worker_section_count - deleted_worker_section_count) > 1) {
        ...
        $("#worker_name_" + index).attr('hidden', 'hidden');
        $("#worker_work_type_id_" + index).attr('hidden', 'hidden');
...
function workerProspectDelete(index) {
    if ((prospect_count - deleted_prospect_count) > 1) {
        ...
        $("#prospect_campus_" + index).attr('hidden', 'hidden');
...
function addProspectDetail() {
    ...
    add_prospect_row += "<div class='row'>";
    add_prospect_row += "<div class='col-3' id='prospect_campus_div_" + prospect_count + "'>";
    add_prospect_row += "</div>";
    add_prospect_row += "<div class='col-3' id='prospect_course_div_" + prospect_count + "'>";
    add_prospect_row += "</div>";
    add_prospect_row += "<div class='col-3' id='prospect_affiliated_body_div_" + prospect_count + "'>";
    add_prospect_row += "</div>";
    add_prospect_row += "<div class='col-3' id='prospect_degree_type_div_" + prospect_count + "'>";
    add_prospect_row += "</div>";
    add_prospect_row += "</div>";
    add_prospect_row += "</div>";
    add_prospect_row += "<div class='col-3 element-flex-end'>";
    ...
    getOrganizationCampusesForProspect(prospect_count);
    prospect_count++;
}
...
function addWorkerSection() {
    ...
    add_worker_detail_row += "<div class='col-md-3'>";
    add_worker_detail_row += "<label>Worker Name:<span style='color: red'>*</span></label>";
    add_worker_detail_row += "<input type='text' name='worker_name' errorLabel='Worker Name ( Row " + (worker_section_count + 1) + " )' onkeypress='return alphabaticOnly(event)' id='worker_name_" + worker_section_count + "' class='form-control letter_capitalize item-required' placeholder='Worker Name'>";
    add_worker_detail_row += "<span id='worker_name_msg_" + worker_section_count + "' hidden='hidden' style='color: red'>Worker Name Required</span>";
    add_worker_detail_row += "</div>";
    add_worker_detail_row += "<div class='col-md-3'>";
    ...
    add_worker_detail_row += "</select>";
    add_worker_detail_row += "</div>";
    add_worker_detail_row += "<div class='col-md-3 pt-3 element-flex-end'>";
...
function onFollowupStatusSelect() {
    ...
    if (selected_status_id == 'Follow Up Required') {
        ...
        document.getElementsByClassName('pwwbFileInformationDiv').hidden = true;
        ...
        document.getElementById('followup_interested_level_div').hidden = true;
        document.getElementById('next_followup_date_id').hidden = true;
        document.getElementsByClassName('follow_up_interested_level_id').hidden = true;
        // if(document.getElementById('student_category_id').value == 0 && document.getElementById('student_category_id').value !== '') {
        //     document.getElementById('pwwbFileInformationDiv').hidden = false;
        // }
...
function checkNumberDuplicacy() {
    ...
    //     }
    // }
    enquiry.phone = document.getElementById('phone1') != null ? document.getElementById('phone1').value : '';
...
function createFormObject(userConsent) {
    var notify = $.notify('Configuring your enquiry request.', 'message', {
        clickToHide: true,
        arrowShow: true,
        autoHide: false,
    });
    ...
    enquiry.phone1 = document.getElementById('phone1') != null ? document.getElementById('phone').value : '';
    enquiry.phone2 = document.getElementById('phone2') != null ? document.getElementById('phone').value : '';
    enquiry.landline = document.getElementById('landline') != null ? document.getElementById('landline').value : '';
    enquiry.project_id = document.getElementById('project_id') != null ? document.getElementById('project_id').value : '';
    enquiry.product_id = document.getElementById('product_id') != null ? document.getElementById('product_id').value : '';
    enquiry.developer_id = document.getElementById('developer_id') != null ? document.getElementById('developer_id').value : '';
    enquiry.price_offer = document.getElementById('price_offer') != null ? document.getElementById('price_offer').value : '';
    ...
    // enquiry.is_domicile = document.getElementById('is_domicile') != null ? document.getElementById('is_domicile').value : '';
    ...
    // }
    // if (document.getElementById('prospects_id') != null && document.getElementById('prospects_id').value == 0) {
    //     for (var i = 0; i < prospect_count; i++) {
    //         if ($("#prospect_section_" + i).attr('row_state') != 'DELETED') {
    //             var prospect = {};
    //             prospect.prospect_name = document.getElementById('prospect_name_' + i) != null ? document.getElementById('prospect_name_' + i).value : '';
    //             prospect.prospect_relationship = document.getElementById('prospect_relationship_' + i) != null ? document.getElementById('prospect_relationship_' + i).value : '';
    //             prospect.prospect_campus = document.getElementById('prospect_campus_' + i) != null ? document.getElementById('prospect_campus_' + i).value : '';
    //             prospect.prospect_course = document.getElementById('prospect_course_' + i) != null ? document.getElementById('prospect_course_' + i).value : '';
    //             prospect.prospect_affiliated_body_id = document.getElementById('prospect_affiliated_body_id_' + i) != null ? document.getElementById('prospect_affiliated_body_id_' + i).value : '';
    //             prospect.prospect_affiliated_body_name = document.getElementById('prospect_affiliated_body_id_' + i) != null ? document.getElementById('prospect_affiliated_body_id_' + i).options[document.getElementById('prospect_affiliated_body_id_' + i).options.selectedIndex].innerText : '';
    //             prospect.prospect_followup_status_id = document.getElementById('prospect_followup_status_id_' + i) != null ? document.getElementById('prospect_followup_status_id_' + i).value : '';
    //             prospect.prospect_academic_term_id = document.getElementById('prospect_academic_term_id_' + i) != null ? document.getElementById('prospect_academic_term_id_' + i).value : '';
    ...
    // enquiry.student_cell_no = document.getElementById('student_cell_no')!= null ?document.getElementById('student_cell_no').value:'';
    // enquiry.father_cell_no = document.getElementById('father_cell_no')!= null ?document.getElementById('father_cell_no').value:'';
    // enquiry.gaurdian_cell_no = document.getElementById('gaurdian_cell_no')!= null ?document.getElementById('gaurdian_cell_no').value:'';
    // enquiry.other_cell_no = document.getElementById('other_cell_no')!= null ?document.getElementById('other_cell_no').value:'';
    // enquiry.course_id = document.getElementById('course_id') != null ? document.getElementById('course_id').value : '';
    // enquiry.course_name = document.getElementById('course_id') != null ? document.getElementById('course_id').options[document.getElementById('course_id').options.selectedIndex].innerText : '';
    ...
    enquiry.other_city_name = document.getElementById('other_city_name') != null ? document.getElementById('other_city_name').value : '';
    // enquiry.previous_degree_id = document.getElementById('previous_degree_id') != null ? document.getElementById('previous_degree_id').value : '';
    ...
    if (move_to_followup) {
        if (!document.getElementById('follow_up_interested_level_id').hidden) {
            enquiry.follow_up_interested_level_id = document.getElementById('follow_up_interested_level_id') != null ? document.getElementById('follow_up_interested_level_id').value : '';
        }
        enquiry.next_followup_date_id = document.getElementById('next_followup_date_id') != null ? document.getElementById('next_followup_date_id').value : '';
    }
    ...
    enquiry.status_id = 0;
    enquiry.form_bypassed = this.form_bypassed;
...
function saveEnquiry() {
    var notify = $.notify('Please Wait! Saving Form.', 'message', {
        clickToHide: true,
        arrowShow: true,
        autoHide: false,
    });
