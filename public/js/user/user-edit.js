...
function onExperienceLevelSelect() {
    ...
    $("#hourly_rate_range_field").html("");
    console.log(constants.hourly_rates);
...
$('#campus_id').on("select2:select", function(e) {
    $.ajax({
        url: "/users/addUserCampusDetails/" + e.params.data.id,
        // dataType: "json",
        type: "GET",
...
$('#campus_id').on("select2:unselect", function(e) {
    $('.' + e.params.data.text.replace(' ', '')).html('');
});
...
//     $.ajax({
//         url: "../departments/getDepartmentDesignations/" + e.target.selectedOptions[0].value,
//         // dataType: "json",
//         type: "GET",
//         success: function(data) {
...
//                 if (index == 0) {
//                     $('#' + e.target.attributes.name_for_ids.value + '_designation').html($('<option></option>').html('--- Select Designation ---'));
