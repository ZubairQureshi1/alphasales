...
function getFilteredData() {
  ...
  $.ajax({
    ...
    url: "reportings/getReportingData",
    type: "POST",
    data: {
...
document.getElementById('followup_count').innerHTML=("Total :"+ response_data.length);
      for (var i = 0; i < response_data.length; i++) {
        
var Mydate = new Date( response_data[i].date.date).toISOString().slice(0,10);


         var add_row = "<tr>";
        add_row += "<td>" +Mydate + "</td>";
        add_row += "<td>" + response_data[i].admitted + "</td>";
        add_row += "<td>" + response_data[i].interested + "</td>";
        add_row += "<td>" + response_data[i].notInterested + "</td>";
        add_row += "</tr>";
        $('#course_wise_report').append(add_row);
      }
   var add_row = "<tr>";
        add_row += "<td><b> TOTAL </b></td>";
        add_row += "<td>" + data.admitted + "</td>";
        add_row += "<td>" + data.interested+ "</td>";
        add_row += "<td>" + data.notInterested + "</td>";
        add_row += "</tr>";
        $('#course_wise_report').append(add_row);
