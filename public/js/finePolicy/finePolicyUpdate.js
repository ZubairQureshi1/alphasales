$(function() {
	...
	$('#editButtonTest').click(function() {
		var fine_policies = {
			policy_id: document.getElementById('policy_id') != null ? document.getElementById('policy_id').value : '',
		...
		if (fine_policies.wing_id != 2) {
			for (var i = 0; i < edit_credit_hour; i++) {
				if($('#editCreditCourse_'+i).attr('row_status') !== 'deleted') {
					let credit_hours = {
		...
		$.ajax({
			url: '/studentAttendancePolicy/updateFilePolicy',
			type: 'POST',
			data: {
			...
			success: function(resp) {
                swal({
                    title: 'Fine Policy Updated Successfully!',
                    type: 'success',
