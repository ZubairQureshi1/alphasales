...
class UpdateStudentCSRollNumberSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        foreach ($students as $key => $student) {
            ...
            if ($course->courseSessions()->where(['organization_campus_id' => $student->organization_campus_id, 'session_id' => $student->session_id])->get()->last()->wing_id == 2) {
                ...
                $system_roll_number->update($system_roll_no_input);
                ...
                if (\File::exists(public_path(config('constants.attachment_path.file_destination_path') . '/Students/' . $old_roll_numbers->where('student_id', $student->id)->last()->roll_no))) {
                    ...
                    $dest_dir = public_path(config('constants.attachment_path.file_destination_path') . '/Students/' . $student->roll_no);
