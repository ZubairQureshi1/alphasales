    ...
    <!-- Page title -->
        <ul class="list-inline menu-left mb-0">
            ...
            <li class="hide-phone list-inline-item app-search">
                <h3 class="page-title">Whatsapp Messages</h3>
            </li>
        </ul>
...
<div class="page-content-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="form-group">
                    <button class="btn btn-primary btn-sm waves-effect waves-light pull-right m-b-10 m-t-15-negative ml-1" onclick="checkSelected()" type="button">
                        <i class="fa fa-send">
                        </i>
                        Send Message
                    </button>
                    <button class="btn btn-primary btn-sm waves-effect waves-light pull-right m-b-10 m-t-15-negative" data-target="#addConcat" data-toggle="modal" type="button">
                        <i class="fa fa-plus-circle">
                        </i>
                        Create
                    </button>
                    <button class="btn btn-primary btn-sm waves-effect waves-light pull-right m-b-10 m-t-15-negative mr-1" onclick="checkSelectedForGroup()" data-target="#addGroup" data-toggle="modal" type="button">
                        <i class="fa fa-plus-circle">
                        </i>
                        Group
                    </button>
                    <button class="btn btn-primary btn-sm waves-effect waves-light pull-right m-b-10 m-t-15-negative mr-1" data-target="#importContacts" data-toggle="modal" type="button">
                            ...
                            <tr id="{{$value['id']}}">
                                ...
                                <td>                                                
                                    <button class="btn btn-warning btn-sm" 
                                            data-toggle="modal"
                                            data-target="#edit_{{$value['id']}}"
                                            >
                                            <i class="mdi mdi-pencil"></i>
                                    </button>
                                </td>
                            </tr>
                            @include('whatsapp.edit-contact')
                            @endforeach
                        </table>
                    </div>
                </div>
        ...
        @include('whatsapp.add-contact')
        @include('whatsapp.import')
        @include('whatsapp.add-to-group')
    </div>
</div>
...
<script type="text/javascript">
    ...
    function checkSelectedForGroup(){
        var selectedrow = contactsTable.rows({ selected: true } ).ids().toArray();
        // console.log("contactsTable" , selectedrow);
        if(selectedrow.length > 0){
            $("#selectContacts").val(selectedrow.join(','));
            $("#addGroup").modal("show");
        }else{
            alert("Please Select Contacts To Send Message");
        }
