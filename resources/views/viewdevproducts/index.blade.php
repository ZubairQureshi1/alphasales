        ...
        <div class="row">
            <div class="col-12">
                <div class="card m-b-30">
                    <div class="card-body"> 
                        <h2>Developer wise Project/Product View</h2>  
                        <div style="height: 500px;overflow: scroll;">
                            <ul class="tree">
                            <!-- org -->
                            @foreach ($org as $index => $rg)
                              <li><div class="sticky">{{ $rg['name'] }}</div>
                                    <ul>
                                    @foreach ($rg['offices'] as $off)
                                        <li><div class="sticky">{{ $off['name'] }} </div> 
                                            @if(isset($off['developers']))
                                                <ul>
                                                @foreach ($off['developers'] as $dev)
                                                    <li><div class="sticky">{{ $dev['name'] }} </div>
                                                        @if(isset($dev['projects']))
                                                            <ul>
                                                            @foreach ($dev['projects'] as $proj)
                                                                <li><div class="sticky">{{ $proj['name'] }} </div>
                                                                    @if(isset($proj['products']))
                                                                        <ul>
                                                                        @foreach ($proj['products'] as $prod)
                                                                        <li><div class="sticky">{{ $prod['name'] }}</div>
                                                                        </li>
                                                                        @endforeach
                                                                        </ul>        
                                                                    @endif 
