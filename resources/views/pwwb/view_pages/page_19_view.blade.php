...
@if(isset($data['third_semester_details']['cell_status']) != null )
    <div class="card shadow p-3 w-100">
        <div class="col-md-12">
            <h1>Continue 2(Third Semester)</h1>
            </div><br>
        <div class="card-body ">
            <div class="card shadow p-3 w-100">
                <div class="card-body">
                    <div class="col-md-12 mt-4">
                        <label for="" class="styling">3rd Semester File Received in CFE Cell:</label>
                    </div>
                    <div class="card shadow p-3 w-100">
                        <div class="card-body ">
                            <div class="form-row">
    ...
    <div class="card shadow p-3 w-100">
        <div class="card-body ">
            <div class="form-row">
                <div class="form-group  col-md-2">
                    <label @if( !$data['claims'][24]['claim_due']) class="text-danger" @endif><strong>Claim Due:</strong></label>
                    <label>
                        {{$data && $data['claims'][24]['claim_due'] ? $data['claims'][24]['claim_due'] : '--'}}
                    </label>
                </div>
                <div class="form-group  col-md-2">
                    <label @if( !$data['claims'][24]['claim_status']) class="text-danger" @endif><strong>Claim Status:</strong></label>
                    <label>
                        {{$data && $data['claims'][24]['claim_status'] ? $data['claims'][24]['claim_status'] : '--'}}
                    </label>
                </div>
                <div class="form-group  col-md-2">
                    <label @if( !$data['claims'][24]['reason']) class="text-danger" @endif><strong>Reason:</strong></label>
                    <label>
                        {{$data && $data['claims'][24]['reason'] ? $data['claims'][24]['reason'] : '--'}}
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-2"></div>
                 <div class="form-group col-md-2">
                    <label><strong>{{-- Type of Claim --}}</strong></label>
                    <label @if( !$data['claims'][24]['total_amount_due']) @endif><strong>Amount Due:</strong></label>
                    <label>
                       {{$data['claims'][24]['total_amount_due']? $data['claims'][24]['total_amount_due'] : '0'}}
                    </label>
                </div>
                 <div class="form-group col-md-2">
                    {{-- <label><strong>Claim Type:</strong></label> --}}
                    <label @if( !$data['claims'][24]['total_amount_received']) @endif><strong>Total Received:</strong></label>
                    <label>
                       {{$data['claims'][24]['total_amount_received']? $data['claims'][24]['total_amount_received'] : '0'}}
                    </label>
                </div>
                <div class="form-group col-md-2">
                    {{-- <label><strong>Status of Claim Received:</strong></label> --}}
                    <label @if( !$data['claims'][24]['total_amount_balance']) @endif><strong>Total Balance:</strong></label>
                    <label>
                       {{$data['claims'][24]['total_amount_balance']? $data['claims'][24]['total_amount_balance'] : '0'}}
                    </label>
                </div>
                        ...
                        @foreach($data['third_semester_result_status_details'] as $thirdSemesterResultStatusDetails)
                            <div class="form-row mt-2" id="result_status_second_semester_div">
                                ...
                                <div class="col-md-1 p-0">
                                    <label id="checkpassorfail_19" style="margin-left: 40px;">
                                        {{$thirdSemesterResultStatusDetails['result'] ? $thirdSemesterResultStatusDetails['result'] : ''}}
                                    </label>
                                </div>
                                <div id="fail_19" class="col-md-10 form-row m-0" id="result_status_second_semester_pass_values" style="display: none">
                    ...
                    <div class="card-body ">
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                <label @if( !$data['third_semester_details']['readmissionthird']) class="text-danger" @endif><strong>Readmission:</strong></label>
                                <label>
                                    {{$data && $data['third_semester_details']['readmissionthird'] ? $data['third_semester_details']['readmissionthird'] : '--'}}
                                </label>
                            </div>
                            <div class="form-group col-md-4">
                                <label @if( !$data['third_semester_details']['same_course']) class="text-danger" @endif><strong>Same Course:</strong></label>
                                <label>
                                    {{$data && $data['third_semester_details']['same_course'] ? $data['third_semester_details']['same_course'] : '--'}}
                                </label>
                            </div>
                            <div class="form-group col-md-4">
                                <label @if( !$data['third_semester_details']['changed_course']) class="text-danger" @endif><strong>Changed Course:</strong></label>
                                <label>
                                    {{$data && $data['third_semester_details']['changed_course'] ? $data['third_semester_details']['changed_course'] : '--'}}
                                </label>
                            </div>
                        </div>
                    </div>
...
<script>
    $( document ).ready(function() {
       var checkpassorfail = $('#checkpassorfail_19').text();
