<?php


namespace App\Fields;


class DocumentAttachmentDetailFields
{
    const TABLE_NAME = 'document_attachment_details';
    const ID = 'id';
    const INDEX_TABLE_ID = 'index_table_id';

    const RESULT_CARD_QUANTITY = 'result_card_quantity';
    const RESULT_CARD_ATTESTED = 'result_card_attested';
    const NOC_AFFILIATED_BODY = 'noc_affiliated_body';
    const EQUIVALENCE_CERTIFICATE = 'equivalence_certificate';
    const CERTIFICATE_QUANTITY = 'certificate_quantity';
    const CHARACTER_CERTIFICATE_ATTESTED = 'character_certificate_attested';
    const COLLAGE_CARD_QUANTITY = 'collage_card_quantity';
    const TRANSPORT_CARD_QUANTITY = 'transport_card_quantity';
    const ADMISSION_LETTER_ORIGINAL = 'admission_letter_original';
    const ADMISSION_LETTER_PRINCIPAL_SIGN = 'admission_letter_principal_sign';
    const BONAFIDE_LETTER_ORIGINAL = 'bonafide_letter_original';
    const BONAFIDE_LETTER_PRINCIPAL_SIGN = 'bonafide_letter_principal_sign';
    const CLAIM_LETTER_ORIGINAL = 'claim_letter_original';
    const CLAIM_LETTER_PRINCIPAL_SIGN = 'claim_letter_principal_sign';
    const AFFIDAVIT_ORIGINAL = 'affidavit_original';
    const AFFIDAVIT_WORKER_SIGN = 'affidavit_worker_sign';
    const WORKER_THUMB = 'worker_thumb';
    const OATH_COMMISSION_ATTESTED = 'oath_commission_attested';


    const CREATED_AT = 'created_at';
    const UPDATED_AT = 'updated_at';

    // ALi Naeem Edit.
    const NOC_RECIEVED_YES_NO = 'noc_received_yes_no';
    const EQUIVALENCE_YES_NO = 'equivalence_yes_no';
    const BONAFIDE_RECIEVED_YES_NO = 'bonafide_recieved_ves_no';
}
