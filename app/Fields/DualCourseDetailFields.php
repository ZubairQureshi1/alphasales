<?php


namespace App\Fields;


class DualCourseDetailFields
{
    const TABLE_NAME = 'dual_course_details';
    const ID = 'id';
    const INDEX_TABLE_ID = 'index_table_id';
    const COURSE = 'course';
    const ROLL_NO = 'roll_no';
    const AFFILIATED_BODY = 'affiliated_body';
    const DURATION_OF_COURSE = 'duration_of_course';
    const ADMISSION_DATE = 'admission_date';
    const ENDING_DATE = 'ending_date';
    const SCHEME_OF_STUDY = 'scheme_of_study';
    const SEMESTER_CATEGORY= 'semester_category';
    const SHIFT = 'shift';
    const REGISTRATION_STATUS = 'registration_status';
    const REGISTRATION_DATE = 'registration_date';
    const ACTUAL_FEE = 'actual_fee';
    const LATE_FEE = 'late_fee';
    const TOTAL_FEE = 'total_fee';
    const BOARD_UNIVERSITY = 'board_university';
    const PREVIOUS_COURSE = 'previous_course';
    const PREVIOUS_ROLL_NO = 'previous_roll_no';
    const PREVIOUS_PASSING_DATE = 'previous_passing_date';
    const PREVIOUS_TOTAL_MARKS = 'previous_total_marks';
    const PREVIOUS_MARKS_OBTAINED = 'previous_marks_obtained';
    const PREVIOUS_PERCENTAGE = 'previous_percentage';
    const PREVIOUS_CGPA = 'previous_cgpa';
    const CREATED_AT = 'created_at';
    const UPDATED_AT = 'updated_at';
}
